(() => {
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        ...
        const menuArrows = document.querySelectorAll(".mobile-menu-arrow");
        let isDesktop = window.innerWidth >= 992 || window.screen.width >= 992;
        ...
        document.addEventListener("click", (function(event) {
            ...
            openSubmenus.forEach((menuItem => {
                const subMenu = menuItem.querySelector(".sub-menu");
                if (!menuItem.contains(event.target) || event.target !== menuItem.querySelector(".mobile-menu-arrow") && !subMenu.contains(event.target)) {
        ...
        function resetMenuStyles() {
            menuItems.forEach((menuItem => menuItem.classList.remove("submenu-open")));
        ...
        function toggleSubMenu() {
            subMenus.forEach((subMenu => subMenu.hidden = true));
            menuItems.forEach((menuItem => menuItem.classList.remove("submenu-open")));
        }
        function handleMenuState() {
            isDesktop = window.innerWidth >= 992 || window.screen.width >= 992;
            isTouchDevice = window.matchMedia("(any-pointer: coarse)").matches;
            if (isDesktop && !isTouchDevice) resetMenuStyles(); else if (isDesktop && isTouchDevice) toggleSubMenu(); else if (!isDesktop && isTouchDevice) toggleSubMenu();
        }
        handleMenuState();
        let resizeTimer;
        window.addEventListener("resize", (function() {
            clearTimeout(resizeTimer);
            resizeTimer = setTimeout(handleMenuState, 200);
        }));
    }));
    addLoadedClass();
    menuInit();
